...
int push_client_to_vec(struct sockaddr_in client_addr[MAX_CLIENT],
                       struct sockaddr_in *addr, int nof_client) {
...
void *dci_sink_server_thread(void *p) {
  ...
  int n;
  len = sizeof(cliaddr);
  // we try to build the connection first: wati for the client to send the
  // connection request to us
  while (!go_exit) {
    // recv data from client and get the client address
    n = recvfrom(sockfd, (char *)recvBuf, 1400, MSG_WAITALL,
                 (struct sockaddr *)&cliaddr, &len);
    if (n > 0) {
      printf("PORT: %d recv len:%d | %d %d \n", cliaddr.sin_port, n, recvBuf[0],
             recvBuf[1]);
      ...
      if (recvBuf[0] == (char)0xCC && recvBuf[1] == (char)0xCC &&
        ...
        // push the client to the local list | skip if the client is already
        // inside the list
        ...
        if (new_client > nof_client) {
          ...
          sendto(sockfd, (char *)recvBuf, 4, MSG_CONFIRM,
      ...
      if (recvBuf[0] == (char)0xFF && recvBuf[1] == (char)0xFF &&
          recvBuf[2] == (char)0xFF && recvBuf[3] == (char)0xFF) {
        // TODO finish
      }
    }
    // sleep
