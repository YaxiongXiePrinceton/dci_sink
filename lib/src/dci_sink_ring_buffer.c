...
uint16_t find_dci_cell_tail(ngscope_dci_sink_cell_t *q) {
  ...
  // printf("nof_dci:%d header:%d | %d tail:%d | %d ", nof_dci, header,
  // q->dci[header].tti, tail_idx, q->dci[tail_idx].tti);
...
void insert_single_dci(ngscope_dci_sink_CA_t *q, ue_dci_t *ue_dci) {
  ...
  if (((prev_tti + 1) % 10240) != curr_tti) {
    printf("We missing at least one DCI message! prev:%d curr:%d ", prev_tti,
           curr_tti);
